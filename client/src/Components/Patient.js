...
function Patient(props) {
    ...
    async function loadFiles() {
        const files = await contract.methods.getUserFiles(accounts[0]).call({ from: accounts[0] });
        console.log('files',files);
        if (files[0]) setFiles(files);
    }

    async function loadPatient () {
        let res = await contract.methods.getPatientInfo().call({ from: accounts[0] });
        setName(res[0]);
        setAge(res[2]);
        setFiles(res[3]);
        setDoctorList(res[4]);
    ...
    async function grantAccess() {
        if (doctorId) {
            let res = await contract.methods.grantAccessToDoctor(doctorId).send({ "from": accounts[0] });
            if (res) {
                message.success('access successful');
                setDoctorId(null);
            }
    ...
    async function updateFileHash(name,type,ipfshash) {
        //sending transaction and storing result to state variables
        let res = await contract.methods.addUserFiles(name,type,ipfshash).send({"from":accounts[0]});
        console.log(res);
        ...
        <div>
          <Row gutter={16} style={{ display: "flex", flexDirection: "row", justifyContent: "space-between" }}>
            ...
            <Col className="col-3" span={6}>
              <Card bordered={true}>
                <div style={flexStyle}>
                  <Input className="emailId" style={{ width: "100%" }} value={doctorId} onChange={(e) => onTextChange("doctorId", e)} size="small" placeholder="Doctor Address" />
                  <Button type="primary" onClick={grantAccess} htmlType="submit" className="login-form-button loginButton">
                    Grant Access
                  </Button>
                </div>
              </Card>
